...
export default function Service() {
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      // 📱 MOBILE/TABLET — show flipped cards (no scroll animation)
      // Breakpoint matches Tailwind's `lg` (1024px) exactly
  ...
  return (
    <main ref={containerRef} className="font-['Instrument_Serif']">
      <section className="stickies h-svh flex items-center justify-center px-8 max-lg:h-auto max-lg:flex-col max-lg:justify-start max-lg:gap-10 max-lg:py-16">
        <div className="sticky-header absolute top-[20%] left-1/2 -translate-x-1/2 -translate-y-1/2 max-lg:static max-lg:transform-none max-lg:w-full">
          <h1 className="text-[4rem] opacity-0 translate-y-10 text-center max-lg:opacity-100 max-lg:translate-y-0 max-lg:text-3xl">
            What am I good at?
          </h1>
        </div>

        <div className="card-container w-[75%] flex perspective-[1000px] translate-y-10 max-lg:w-full max-lg:flex-col max-lg:items-stretch max-lg:gap-6 max-lg:translate-y-0">
          {[
            ...
            <div
              key={id}
              id={id}
              className={`card relative flex-1 aspect-5/7 transform-style-preserve-3d origin-top max-lg:flex-none max-lg:w-[55%] max-lg:h-auto ${
                i % 2 === 1 ? "max-lg:self-end" : "max-lg:self-start"
              }`}
            >
              {/* Front */}
              ...
              {/* Back */}
              <div
                className={`absolute inset-0 backface-hidden rotate-y-180 flex items-center justify-center text-center p-6 sm:p-8 rounded-[20px] ${bg}`}
              >
                <span className="absolute top-6 left-6 sm:top-8 sm:left-8 opacity-40">
